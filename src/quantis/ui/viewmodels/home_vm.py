...
class HomeViewModel(BaseViewModel):
    ...
    async def _load_async(self) -> None:
        ...
        try:
            ...
            recent, downloaded, user_playlists = await asyncio.gather(
                recent_task,
                downloaded_task,
                playlists_task,
            )
    ...
    def _build_snapshot(
        ...
        *,
        ...
        if recent_tracks:
            library_playlists.append(RecentlyPlayedPlaylist(tracks=recent_tracks))
        download_pl = DownloadPlaylist(tracks=downloaded)
        if download_pl.tracks.values:
            library_playlists.append(download_pl)
    ...
    async def play_playlist(self, playlist: Playlist, start_index: int = 0) -> None:
        ...
        if isinstance(playlist, RecentlyPlayedPlaylist):
            ...
            working = Playlist(playlist.name, tracks, playlist.cover_path)
